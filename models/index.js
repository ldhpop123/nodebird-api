...
const path = require('path') // path 모듛 가져오기
...
db.sequelize = sequelize; // Seqelize 인스턴스를 db 객체에 추가

// 수동 연결
/*
db.User = User; // User 모델을 db 객체에 추가
db.Post = Post; // Post 모델을 db 객체에 추가
db.Hashtag = Hashtag 모델을 db 객체에 추가

User.initiate(sequelize); // User 모델 초기화
Post.initiate(sequelize); // Post 모델 초기화
Hashtag.initiate(sequelize); // Hashtag 모델 초기화

User.associate(db); // User 모델과 연관 관계 설정
Post.associate(db); // Post 모델과 연관 관계 설정
Hashtag.associat(db); // Hashtag 모델과 연관 관계 설정
*/
...
fs
  ...
  // (file.indexOf('.') !== 0): 파일 이름일 '.'으로 시작하지 않는 파일을 필터링
...
Object.keys(db).forEach(modelName => { // 모델의 연관 관계 설정
